class Graph:
    ...
    def new_edge(self, origin, destiny, weight):
        ...
        # If the origin or destiny nodes do not exist, initialize them with empty lists.
        if origin not in self.content:
            self.content[origin] = []
        if destiny not in self.content:
            self.content[destiny] = []
        # Append the new edge to the origin node's list.
        self.content[origin].append((destiny, weight))

    def view_all(self):
        # Print all edges in the graph, showing connections and their weights.
        print("\n\nGraph:\n------")
